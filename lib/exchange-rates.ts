export const currentExchangeRate = {
  usdToEtb: 56.8, // Base fallback rate
  eurToEtb: 62.5,
  gbpToEtb: 72.3,
  sarToEtb: 15.1,
  aedToEtb: 15.5,
...
// Ethiopian Bank-specific exchange rates (simulated +/- 0.5% variation)
type BankRate = { name: string; usdToEtb: number; lastUpdate: Date };

export const getBankExchangeRates = async (): Promise<
  Record<string, BankRate>
> => {
  const baseUsdToEtb = currentExchangeRate.usdToEtb;

  // Simulate slight variances like in real banking
  const bankRates: Record<string, BankRate> = {
    cbd: {
      name: "Commercial Bank of Ethiopia (CBE)",
      usdToEtb: baseUsdToEtb - 0.2,
      lastUpdate: new Date(),
    },
    dashen: {
      name: "Dashen Bank",
      usdToEtb: baseUsdToEtb - 0.1,
      lastUpdate: new Date(),
    },
    awash: {
      name: "Awash Bank",
      usdToEtb: baseUsdToEtb + 0.15,
      lastUpdate: new Date(),
    },
    abyssinia: {
      name: "Bank of Abyssinia",
      usdToEtb: baseUsdToEtb + 0.3,
      lastUpdate: new Date(),
    },
    nib: {
      name: "Nib International Bank",
      usdToEtb: baseUsdToEtb - 0.4,
      lastUpdate: new Date(),
    },
    wegagen: {
      name: "Wegagen Bank",
    ...
    oromia: {
      name: "Oromia Bank",
      usdToEtb: baseUsdToEtb + 0.25,
      lastUpdate: new Date(),
    },
  };

  // Try to fetch real ETB rates and apply variations
  try {
    if (typeof window !== "undefined") {
      await fetchRealExchangeRate("USD");
    }
    // Update rates with fresh base rate
    const updatedBase = currentExchangeRate.usdToEtb;
    Object.keys(bankRates).forEach((bank) => {
      const original = bankRates[bank];
      const variation = original.usdToEtb - baseUsdToEtb; // Preserve the bank-specific adjustment
...
export const currencyInfo: Record<
  Currency,
  { name: string; symbol: string; flag: string; region: string }
> = {
...
> {
  ...
  const needsUpdate = currencies.some(
    (currency) => now - lastFetchTimes[currency] >= CACHE_DURATION
  );
...
export async function fetchRealExchangeRate(
  ...
  try {
    ...
    console.log(
      `[v0] Using fallback rate: 1 ${currency} = ${fallbackRate} ETB`
    );
    return fallbackRate;
  } finally {
