...
export default function RequestDetailPage() {
  ...
  const loadRequest = async () => {
    try {
      ...
      console.log("[v0] Request detail API response:", data);
      if (data.success) {
        setRequest(data.data);
        console.log("[v0] Loaded request:", data.data);
      } else {
        console.error("[v0] Request not found:", data.error);
      }
    } catch (error) {
    ...
    <div className="container mx-auto space-y-6 p-4 pb-20 md:pb-6">
      <div>
        ...
        <p className="text-muted-foreground">
          View and manage this payment request
        </p>
      </div>
